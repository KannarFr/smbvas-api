...
class ResourceController @Inject()(
  ...
  def getValidatedResources = Action.async { implicit request: Request[AnyContent] =>
    Logger.info(request.headers.get("X-Forwarded-For").toString)
    analyticsDAO.pushAccessFor(request.headers.get("X-Forwarded-For").get)
  ...
  def validateResourceById(resourceId: UUID) = authenticatedAction.async { implicit request =>
    Future {
  ...
  def unvalidateResourceById(resourceId: UUID) = authenticatedAction.async { implicit request =>
    Future {
